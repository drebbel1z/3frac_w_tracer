...
ax1.plot(
    df["time"] / 3600 / 24,
    df["injection_rate_kg_s"] * kgs_to_bpm*0.433,
    "-b",
    label="Field Injection",
)
ax1.plot(
    df_30day_exp_mass["time"] / 3600 / 24 - 0.8,
    df_30day_exp_mass["m_out_kg_s"] * kgs_to_bpm*0.433,
    linestyle="--",
    linewidth=2,
    color="black",
    label="Field Production",
)
plt.axvline(x=247001/3600 / 24, color="grey", linestyle="--", alpha=0.3, linewidth=3)
plt.axvline(x=9, color="grey", linestyle="--", alpha=0.3, linewidth=3)
# ax1.axhline(0.9472, label="what you want")
ax1.legend()
...
plt.axvline(x=247001, color="grey", linestyle="--", alpha=0.3, linewidth=3)
plt.axvline(x=9*24*3600, color="grey", linestyle="--", alpha=0.3, linewidth=3)
# ax1.set_xlim(-5*24*3600,10*24*3600)
# Creating a second y-axis for MPa
ax2 = ax1.twinx()
y_min, y_max = ax1.get_ylim()
y_min_si = y_min / MPa_to_psi
...
plt.legend()
plt.vlines(x=0,ymin=0, ymax=1, color="grey", linestyle="--", alpha=0.3, linewidth=3)
plt.vlines(x=100000,ymin=0, ymax=1, color="grey", linestyle="--", alpha=0.3, linewidth=3)
